...
static void battery_main(void *, void *, void *) {
	...
	if (rc != 0) {
		LOG_WRN("Failed initialize battery measurement: %d\n", rc);
		return ;
	}
	...
	while (true) {
		...
		sum_of_moveing_average += tmp_mV;
		if (isFull()) {
			sum_of_moveing_average -= dequeue();
		}
		enqueue(tmp_mV);
		
		int batt_mV = sum_of_moveing_average/getItemCount();
